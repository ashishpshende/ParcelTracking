...
const Nav = class {
  ...
  success(result, ti) {
    ...
    if (ti.done) {
      ti.done(results.hasCompleted, results.requiresTransition, results.enteringView, results.leavingView, results.direction);
    }

    ti.resolve(results.hasCompleted);

    if (ti.opts.updateURL !== false && this.useRouter) {
      const router = document.querySelector('ion-router');

      if (router) {
        const direction = results.direction === 'back' ? 'back' : 'forward';
