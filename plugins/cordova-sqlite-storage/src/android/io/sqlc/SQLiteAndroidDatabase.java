...
class SQLiteAndroidDatabase
{
            ...
            try {
                ...
                if (queryType == QueryType.update || queryType == queryType.delete) {
                    // if (isPostHoneycomb) {
                        ...
                        if (rowsAffected != -1) {
                            ...
                            queryresults.put("rowsAffected", rowsAffected);
                        }
                    // }
                ...
                if (queryType == QueryType.insert && json_params != null) {
                    ...
                    try {
                        ...
                        queryResult = new JSONObject();
                        if (insertId != -1) {
                            queryresults.put("insertId", insertId);
                            queryresults.put("rowsAffected", 1);
                        } else {
                            queryresults.put("rowsAffected", 0);
                        }
                    } catch (SQLiteConstraintException ex) {
                ...
                if (needRawQuery) {
                    ...
                    if (needRowsAffectedCompat) {
                        queryresults.put("rowsAffected", rowsAffectedCompat);
                    }
                }
            } catch (Exception ex) {
                ex.printStackTrace();
    ...
    private JSONObject executeSqlStatementQuery(SQLiteDatabase mydb, String query,
        ...
        if (cur != null && cur.moveToFirst()) {
            ...
            try {
                rowsresults.put("rows", rowsArrayResult);
            } catch (JSONException e) {
                e.printStackTrace();
            }
        }
